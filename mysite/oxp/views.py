...
def raw(request):
    date="time is " + str(datetime.date.today())
    #date="time is " +datetime.date.today()
    print(date)
    #DeleteImage("some_image20190212-212004.jpg")
    ...
    if request.method == "POST":
        ...
        if os.path.exists('static/images'):
            print("yes static/images exists")
            # file.write(os.path.dirname('static/images/'))
            # file.open(file)
            # file.write
            path = os.path.join('static', 'images', str(file))
            img = Image.open(file)
            img.save(path)

        else:
            print("yes static/images does not exist so we are making one...")
            os.mkdir("static/images")
            path = os.path.join('static', 'images', str(file))
            img = Image.open(file)
            img.save(path)
        # return HttpResponse(file.open(file),content_type="image/png")
        # return HttpResponse(file.open('r'),content_type="image/png")
        # return HttpResponse(file.open('r'),content_type="image/png")
        return HttpResponse("POSTTT")
    else:
        return HttpResponse("UNIDENTIFIED REQUEST METHOD IN RAW")
        """
...
"""def category(request):
   if request.method=="GET":
       ...
       return HttpResponse("UNIDENTIFIED REQUEST METHOD IN CATEGORY")"""
